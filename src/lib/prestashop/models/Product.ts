...
export default class Product extends Model {
  ...
  constructor(data: ProductData) {
    ...
    this.available_for_order = data.available_for_order === "1" || data.available_for_order === 1 || data.available_for_order === true;
    this.weight = data.weight ? (typeof data.weight === "string" ? parseFloat(data.weight) : data.weight) : undefined;
    this.name = data.name;
  ...
  getMetaDescription(): string {
    return this.getLanguageValue(this.meta_description) || this.getDescriptionShort();
  }

  static async getBySlug(slug: string): Promise<Product> {
    return this.findOne({ link_rewrite: slug }, { exactMatch: true }) as Promise<Product>;
  }
  ...
  async getCombinations(): Promise<Combination[]> {
    ...
    return combinations.map((data: any) => new Combination(data, this));
  }
  ...
  async getAttributeGroups(): Promise<any[]> {
    ...
    const productValueIds = productOptionValues.map((val) => parseInt(val.id.toString()));
